...
import { isLid, lidToJid, getCachedJid, cacheParticipantLids, decodeAndNormalize } from '../lid.js'
    ...
    const sock = Object.defineProperties(conn, {
        ...
        decodeJid: {
            value(jid) {
                ...
                const decoded = jid.decodeJid();
                if (decoded && isLid(decoded)) {
                    const cached = getCachedJid(decoded) || getCachedJid(decoded.replace('@lid', '@s.whatsapp.net'));
                    if (cached) return cached;
                    return lidToJid(decoded);
                }
                return decoded;
            },
        },
                ...
                for (const message of m) {
                    try {
                        ...
                        if (isGroup) {
                            ...
                            if (chats.metadata?.participants) {
                                cacheParticipantLids(chats.metadata.participants);
                            }
                            ...
                            if (sender && isLid(sender)) {
                                const cached = getCachedJid(sender) || getCachedJid(sender.replace('@lid', '@s.whatsapp.net'));
                                if (cached) sender = cached;
                                else sender = lidToJid(sender);
                            }
                            if (sender !== chat) {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                ...
                if (raw && isLid(raw)) {
                    const cached = getCachedJid(raw) || getCachedJid(raw.replace('@lid', '@s.whatsapp.net'));
                    if (cached) return cached;
                    return lidToJid(raw);
                }
                return raw;
