 ...
 * @param {string} input - Número con +, espacios, guiones, etc.
 ...
 * @param {string|number} number - Número de teléfono
 * @returns {string} JID en formato number@s.whatsapp.net
 */
...
/**
 * Extraer número de un JID
 * @param {string} jid - JID (number@s.whatsapp.net, number@lid, etc.)
 ...
 * @returns {string} Número con prefijo +
 */
function formatDisplay(jid) {
  const num = fromJid(jid);
  return num ? '+' + num : '';
}

/**
 * Validar si es un número de teléfono válido por código de país
 * @param {string} number - Número sin +
 * @returns {boolean}
 */
function isValidPhoneNumber(number) {
  const cleaned = cleanNumber(number);
  if (!cleaned || cleaned.length < 8 || cleaned.length > 15) return false;
  ...
  const validCodes = [
    ...
    "98","212","213","216","218","220","221","234","249","254","255","256",
    "260","263","351","352","353","354","355","356","357","358","359","370",
    "371","372","373","374","375","376","377","378","380","381","382","383",
    "385","386","387","389","420","421","423","852","853","855","856","880",
  ...
  return validCodes.some(code => {
    if (cleaned.startsWith(code)) {
      const localNum = cleaned.slice(code.length);
      return localNum.length >= 6 && localNum.length <= 10;
    }
    return false;
  });
}
...
/**
 * Normalizar cualquier input a JID válido
 * Acepta: número suelto, JID completo, @s.whatsapp.net, @lid
 * @param {string} input - Input del usuario
 * @returns {string} JID válido o string vacío
 */
